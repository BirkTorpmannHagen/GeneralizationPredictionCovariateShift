...
from datasets.wilds_datasets import build_camelyon17_dataset
...
class Camelyon17TestBed(BaseTestBed):
    """WILDS Camelyon17 (domain = hospital). Typicality/Glow detector is skipped."""
    ...
    def __init__(self, model="resnet", mode="normal", sampler="RandomSampler", batch_size=16, pretrained=True):
        ...
        self.ind_train, self.ind_val, self.ind_test, self.ood_val, self.ood_test = \
            build_camelyon17_dataset(WILDS_ROOT, self.trans, self.trans)
        ...
        if model == "resnet":
            self.classifier = ResNetClassifier.load_from_checkpoint(
                f"{prefix}/{model}/Camelyon17/checkpoints/best.ckpt",
        ...
        # Typicality detector is skipped for the WILDS additions (no Glow trained).
        self.glow = None
        self.mode = mode
